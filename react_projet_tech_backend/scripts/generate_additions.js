...
let catId = catStart;
let prodImgId = 1; // we'll insert without id for product_images
let prodTransId = 1; // similarly

// Define categories and subcategories from screenshots
const catalog = [
  {
...
// Brands pool and Russian name transformer (simple)
const brands = [
  'Samsung',
  'Apple',
  'Infinix',
...
function ru(s) {
  return s; /* leave same for simplicity; user can adjust */
}
...
for (const cat of catalog) {
  for (const sub of cat.subs) {
    ...
    sql += `INSERT INTO \`categories\` (id,slug,parent_id,image) VALUES (${catId},'${slug}',${cat.parentId},NULL);\n`;
    sql += `INSERT INTO \`category_translations\` (id,category_id,lang,name) VALUES (${catId * 2 + 1},${catId},'en','${sub.replace(/-/g, ' ')}'),(${catId * 2 + 2},${catId},'ru','${ru(sub.replace(/-/g, ' '))}');\n`;
    catId++;
  }
}

// products: 50 per subcategory
for (const cat of catalog) {
  for (const sub of cat.subs) {
    ...
    for (let i = 1; i <= 50; i++) {
      ...
      sql += `INSERT INTO \`products\` (id,name,category_id,slug,brand_id,specifications,description,short_description,price,discount_price,image,images_gallery,is_active,views,created_at,updated_at,stock) VALUES (${prodId},'${name.replace(/'/g, "\\'")}',(SELECT id FROM categories WHERE slug='${subSlug}'),'${slug}',NULL,NULL,'${name.replace(/'/g, "\\'")}','${sub.replace(/-/g, ' ')}',${price},${discount === 'NULL' ? 'NULL' : discount},'${img}',NULL,1,0,CURRENT_TIMESTAMP(),CURRENT_TIMESTAMP(),100);\n`;
      // product translations
      sql += `INSERT INTO \`product_translations\` (id,product_id,locale,name,description,short_description) VALUES (${prodTransId},${prodId},'en','${name.replace(/'/g, "\\'")}','${name.replace(/'/g, "\\'")}', '${sub.replace(/-/g, ' ')}');\n`;
      sql += `INSERT INTO \`product_translations\` (id,product_id,locale,name,description,short_description) VALUES (${prodTransId + 1},${prodId},'ru','${ru(name.replace(/'/g, "\\'"))}','${ru(name.replace(/'/g, "\\'"))}','${ru(sub.replace(/-/g, ' '))}');\n`;
      prodTransId += 2;
      // product images
      sql += `INSERT INTO \`product_images\` (product_id,image_url,is_main,sort_order) VALUES (${prodId},'${img}',1,0);\n`;
      prodId++;
    }
  }
}
